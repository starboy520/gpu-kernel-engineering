...
Canonical 结果呈现明确 shape 边界：non-causal `D=128` 下，`N=512` 延迟增加 18.2%，`N=768` 近似持平，`N=1024` 延迟降低 17.6%；`D=64` 在 `N=512/768` 明显回退，`N=1024` 仍仅近似持平。ncu 显示 long scoreboard 几乎降为零，但 Shared Memory 从 17,484 B/block 增至 33,912 B/block，active warps 明显下降。该版本证明异步路径有效，但不宣称稳定加速。

...
[[ -x "$runner" ]] || {
    printf 'test_tiled: runner 不存在或不可执行: %s\n' "$runner" >&2
    exit 1
}
...
for item in "${cases[@]}"; do
    ...
    printf '[test_tiled] shape=%sx%s causal=%s input_pattern=%s\n' \
        "$n" "$d" "$causal" "$input_pattern"
    if output="$("$runner" --kernel tiled --n "$n" --d "$d" \
...
printf '[test_tiled] summary executions=%d pass=%d status=PASS\n' \
    "${#cases[@]}" "$passed"

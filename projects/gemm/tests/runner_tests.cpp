...
template <typename T> class ScopedBinding {
  public:
    ...
    ~ScopedBinding() { target_ = previous_; }
...
gemm::LaunchResult fake_reference_launcher(const float *a, const float *b,
    ...
    GPU_CUDA_CHECK(cudaMemcpy(host_a.data(), a, sizeof(float) * a_count,
                              cudaMemcpyDeviceToHost));
    GPU_CUDA_CHECK(cudaMemcpy(host_b.data(), b, sizeof(float) * b_count,
                              cudaMemcpyDeviceToHost));
    ...
    GPU_CUDA_CHECK(cudaMemcpy(c, host_c.data(), sizeof(float) * c_count,
...
cudaError_t counting_free(void *) { return cudaSuccess; }
...
cudaError_t scripted_memcpy_d2h(void *destination, const void *,
                                std::size_t bytes) {
...
cudaError_t counting_peek_at_last_error() { return cudaSuccess; }

cudaError_t counting_device_synchronize() { return cudaSuccess; }

cudaError_t counting_event_create(cudaEvent_t *) { return cudaSuccess; }

cudaError_t counting_event_destroy(cudaEvent_t) { return cudaSuccess; }

cudaError_t counting_event_record(cudaEvent_t, cudaStream_t) {
    return cudaSuccess;
}

cudaError_t counting_event_synchronize(cudaEvent_t) { return cudaSuccess; }
...
cudaError_t host_memcpy(void *destination, const void *source,
                        std::size_t bytes) {
...
std::filesystem::path make_temp_csv_path() {
    return std::filesystem::temp_directory_path() /
           ("gemm_runner_csv_test_" + std::to_string(temp_csv_counter++) +
            ".csv");
}
...
gemm::RunnerOptions make_run_options(gemm::RunMode mode, int warmup,
                                     int iterations, int m, int n, int k) {
...
void test_error_macros_throw_readable_single_evaluation_errors() {
    try {
        GPU_CUDA_CHECK(failing_cuda_call());
        check(false, "GPU_CUDA_CHECK did not throw");
    } catch (const std::runtime_error &error) {
        const std::string message = error.what();
        check(message.find("failing_cuda_call()") != std::string::npos,
              "GPU_CUDA_CHECK includes expression");
        check(message.find("cudaErrorInvalidValue") != std::string::npos,
              "GPU_CUDA_CHECK includes readable status");
        check(message.find("runner_tests.cpp:") != std::string::npos,
              "GPU_CUDA_CHECK includes source location");
    }
    check(cuda_evaluations == 1, "GPU_CUDA_CHECK evaluates expression once");
...
void test_parser_defaults_and_full_command() {
    ...
    const gemm::RunnerOptions options = parse(
        {"gemm_runner", "--kernel", "tiled", "--m", "17", "--n", "19", "--k",
         "23", "--mode", "benchmark", "--warmup", "0", "--iterations", "7",
         "--seed", "4294967295", "--csv", "out.csv"});
    check(options.kernel == "tiled", "parser stores kernel");
    check(options.problem.m == 17 && options.problem.n == 19 &&
              options.problem.k == 23,
          "parser stores dimensions");
    check(options.mode == gemm::RunMode::benchmark,
...
void test_parser_rejects_bad_tokens() {
    ...
    check_throws([] { parse({"gemm_runner", "--seed", "4294967296"}); },
                 "integer overflow for --seed: 4294967296",
                 "unsigned overflow");
    check_throws([] { parse({"gemm_runner", "--mode", "fast"}); },
                 "invalid mode: fast (expected validate or benchmark)",
                 "invalid mode");
}
...
void test_option_validation() {
    ...
    gemm::validate_options(parse({"gemm_runner", "--kernel", "naive", "--m",
                                  "1", "--n", "2", "--k", "3"}));
    gemm::validate_options(
        parse({"gemm_runner", "--kernel", "naive", "--m", "1", "--n", "2",
               "--k", "3", "--csv", "out.csv"}));
    ...
    check_throws(
        [] {
            gemm::validate_options(
                parse({"gemm_runner", "--kernel", "naive", "--m", "1", "--n",
                       "0", "--k", "3", "--csv", "out.csv"}));
        },
        "--n must be a positive integer",
        "CSV path still validates dimensions");
}
...
void test_checked_multiply() {
    ...
    check_throws(
        ...
        "size overflow while computing matrix bytes",
...
void test_registry_contains_author_kernels_then_cublas_baseline() {
    const std::vector<gemm::KernelDescriptor> kernels =
        gemm::registered_kernels();
    ...
    const gemm::KernelDescriptor *register_first =
        gemm::find_kernel("register");
    const gemm::KernelDescriptor *register_second =
        gemm::find_kernel("register");
    ...
    const gemm::KernelDescriptor *vectorized_first =
        gemm::find_kernel("vectorized");
    const gemm::KernelDescriptor *vectorized_second =
        gemm::find_kernel("vectorized");
    ...
    const gemm::KernelDescriptor *cublas_first =
        gemm::find_kernel("cublas-fp32");
    const gemm::KernelDescriptor *cublas_second =
        gemm::find_kernel("cublas-fp32");
...
void test_scoped_binding_restores_previous_pointer() {
    ...
    {
        ScopedBinding previous_binding(active_fake_kernel_state,
                                       &previous_state);
        {
            ScopedBinding scoped_binding(active_fake_kernel_state,
                                         &scoped_state);
...
void test_gpu_free_cli_paths() {
    ...
    check(help_output.str().find(
              "Validate mode reports latency_ms=0 and gflops=0") !=
              std::string::npos,
...
void test_validate_mode_accepts_test_local_kernel_and_reports_pass() {
    ...
    const gemm::KernelDescriptor kernel{"fake_reference",
                                        fake_reference_launcher, false};
    const gemm::RunnerOptions options =
        make_run_options(gemm::RunMode::validate, 2, 4, 2, 3, 4);

    std::ostringstream output;
    const int result =
        runner_internal::run_with_kernel(options, kernel, output);
    check(result == 0,
          "validate mode returns success for matching fake kernel");
    check(state.launches == 3,
          "validate mode runs warmups plus one validation launch");

    const std::string text = output.str();
    check(text.find("kernel=fake_reference path=test_reference shape=2x3x4 "
...
void test_reference_selection_uses_exact_work_boundary() {
    ...
    check(
        runner_internal::uses_cpu_reference(gemm::Problem{1, 1, threshold - 1}),
...
void test_benchmark_mode_runs_validation_then_warmups_and_iterations() {
    ...
    const int result =
        runner_internal::run_with_kernel(options, kernel, output);
    check(result == 0,
          "benchmark mode returns success for matching fake kernel");
    check(state.launches == 9, "benchmark mode performs validation launch, "
                               "warmups, and timed iterations");

    const std::string text = output.str();
    check(text.find("kernel=fake_reference path=benchmark_path shape=3x2x5 "
                    "status=PASS") != std::string::npos,
...
void test_supplied_large_reference_handles_threshold_problem() {
    ...
    check(output.str().find("reference=cublas-pedantic-fp32") !=
              std::string::npos,
...
void test_large_reference_rejects_globally_biased_output() {
    ...
    const gemm::KernelDescriptor kernel{"fake_biased", fake_biased_launcher,
                                        false};
    const gemm::RunnerOptions options =
...
void test_validation_uses_fixed_one_milli_tolerances() {
    ...
    pass_state.delta = 9.0e-4F;
    {
        ...
        check(runner_internal::run_with_kernel(options, kernel, pass_output) ==
                  0,
    ...
    fail_state.delta = 1.2e-3F;
    {
        ...
        check(runner_internal::run_with_kernel(options, kernel, fail_output) !=
                  0,
...
void test_csv_logging_creates_header_appends_rows_and_uses_env_metadata() {
    ...
    ScopedEnvironmentVariable timestamp("GEMM_BENCH_TIMESTAMP",
    ...
    ScopedEnvironmentVariable nvcc(
        "GEMM_NVCC", "Cuda compilation tools, release 12.4, V12.4.131");
    ...
    if (lines.size() == 3) {
        check(
            lines[0] ==
                "timestamp,git_commit,gpu,cuda,nvcc,kernel,path,m,n,k,warmup,"
                "iterations,latency_ms,gflops,passed,max_abs,max_rel,reference",
        ...
        if (validate_row.size() == 18) {
            check(validate_row[0] == "2026-07-11T08:00:00Z",
                  "CSV captures timestamp metadata");
            check(validate_row[1] == "deadbeefcafebabe",
                  "CSV captures git commit metadata");
            check(validate_row[2] == "NVIDIA_A100-SXM4-80GB",
                  "CSV captures GPU metadata");
            check(validate_row[3] == "CUDA_12.4_runtime",
                  "CSV captures CUDA metadata");
            check(validate_row[4] ==
            ...
            check(validate_row[12] == "0.000000" &&
                      validate_row[13] == "0.000000",
                  "validate CSV rows retain zero latency and gflops");
            check(validate_row[14] == "true",
                  "validate CSV row records pass=true");
